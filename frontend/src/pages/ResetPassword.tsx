...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.response?.data?.detail || t('passwordReset.resetError')
      ...
      <div className="min-h-screen bg-background flex items-center justify-center py-12 px-4">
        <Card className="max-w-md w-full mx-auto px-4 sm:px-6">
          <div className="text-center py-8">
            ...
            <h2 className="text-2xl font-bold text-text-main mb-2">{t('passwordReset.invalidToken')}</h2>
            <p className="text-text-secondary mb-6">{t('passwordReset.invalidTokenMessage')}</p>
        ...
        {!success ? (
          <>
            <div className="text-center mb-6 sm:mb-8">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-text-main mb-2">{t('passwordReset.resetTitle')}</h1>
              <p className="text-sm sm:text-base text-text-secondary">{t('passwordReset.resetSubtitle')}</p>
            </div>
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-text-main mb-2">{t('passwordReset.resetSuccess')}</h2>
            <p className="text-text-secondary mb-6">{t('passwordReset.resetSuccessMessage')}</p>
            <Link to="/login">
              <Button variant="primary" className="w-full">
                {t('passwordReset.backToLogin')}
              </Button>
