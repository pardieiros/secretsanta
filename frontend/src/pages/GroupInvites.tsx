...
import { Search, UserPlus, Mail, CheckCircle, Gift, X, Users, Calendar, CalendarDays, XCircle } from 'lucide-react'
...
type TabType = 'search' | 'pending' | 'sent'
...
export default function GroupInvites() {
  ...
  // Get pending invites - always fetch when component mounts
  ...
  // Get group invite details for modal (only if we have a selected invite)
  ...
  const tabs = [
    { id: 'search' as TabType, label: t('groupInvites.search'), icon: Search },
    { id: 'pending' as TabType, label: t('groupInvites.pending'), icon: UserPlus },
    { id: 'sent' as TabType, label: t('groupInvites.sent'), icon: Mail },
  ]

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-text-main mb-2">{t('groupInvites.title')}</h1>
        <p className="text-text-secondary">{t('groupInvites.subtitle')}</p>
      </div>
